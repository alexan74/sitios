    ...
    public function changePass($id = null)
    {
        ...
        if (!empty($user)) {
            if ($this->request->is(['patch', 'post', 'put'])) {
                ...
                if ($data['password']==$data['twopassword']) {
                    if (!$hasher->check($data['password'], $user['password'])) {
                        $user = $this->Users->patchEntity($user, $data);
                        if ($this->Users->save($user)) {
                            $this->Flash->success(__('Ha sido cambiado correctamente.'));
                            return $this->redirect(['action' => 'view',$id]);
                        }
                        $this->Flash->error(__('No pudo cambiar contraseña. Por favor intente de nuevo.'));
                    } else {
                        $this->Flash->error('Ya existe contraseña. Por favor ingresa otra contraseña.');
                    }
                } else {
            ...
            $this->Flash->error('No encuentra el usuario antes de cambiar contraseña');
            return $this->redirect(['action' => 'view',$id]);
        }
