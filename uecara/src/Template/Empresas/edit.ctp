	...
	<div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-secondary">
            	<i class="fas fa-user"></i> Editar Empresa # <?= $empresa->id;?>
              	...
              	<div class="col-sm-12">	
    				<div class="col-sm-9">
                        <label for="cuit" class="control-label">Cuit</label>
                  		<?php echo $this->Form->input('cuit',[ "label"=>false, "class" => "form-control", "id"=>"cuit", "maxlength"=>15]); ?>
                  	</div>
                  	<div class="col-sm-3">
                        <label for="fecha" class="control-label">Fecha</label>
                  		<input type="date" id="fecha" name="fecha" class="form-control" value="<?php echo $empresa->fecha->format('Y-m-d');?>" disabled /> 
    				</div>
              	</div>       
			</div>
			<div class="row">
			...
			<div class="row">
              	<div class="col-sm-12">
              		<div class="col-sm-12">
    					<h4><?= __('Archivos') ?></h4>
    					<div class="col-sm-6">
    						<label for="files" class="control-label">Subir archivos</label>
        				...
        				<?php if (!empty($empresa->tramites)) { ?>
            				<table class="table table-bordered" width="100%" cellspacing="0">
                                <thead>
                                	<tr>
                                		<th width="25%">Archivo subido por la empresa</th>
                                		<th width="10%"></th>
                             		</tr>	
                                </thead>
                                <tbody>
                                    ...
                                    foreach ($empresa->tramites as $tramite) {
                                	    ...
                                	    <tr>
                                		  <td><?= $this->Html->link($archivo->nombre, ['action' => 'download', $archivo->id], ['escape' => false]) ?></td>
                                		  <td><button type="button" class="btn btn-sm btn-danger" data-id="<?=$archivo['id'];?>" onclick="borrar_archivo(this)"><i class="fas fa-times-circle"> </i></button></td>
                                        </tr>
                                		<?php
                                        }
                                	}  ?>
        				...
        				<table class="table table-bordered nominas" width="100%" cellspacing="0">
                            ...
                            <tbody>
                            	<?php if (!empty($empresa->nominas)) {
                                    $n=0; 
                            	    foreach ($empresa->nominas as $nomina) { ?>
                            	    <input name="nominas[<?=$n;?>][id]" type="hidden" value="<?= $nomina->id;?>" />
                            	    <tr>
                            		  <td><input name="nominas[<?=$n;?>][apellido]" type="text" value="<?= $nomina->apellido;?>" /></td>
                            		  <td><input name="nominas[<?=$n;?>][nombre]" type="text" value="<?= $nomina->nombre;?>" /></td>	  
                            		  <td>
                                    	<select name="nominas[<?=$n;?>][categoria]">
                                    		<option value=""></option>
                                    		<?php if (!empty($categorias)) { 
                                    		    foreach ($categorias as $categoria) {
                                    		        echo '<option value="'.$categoria['nombre'].'"';
                                    		        if ($nomina->categoria==$categoria['nombre']) echo " selected ";
                                    		        echo '>'.$categoria['nombre'].'</option>';
                                    		    }
                                    		}?>
                                    	</select>
                                    </td>
                            		  <td class="tcenter"><input name="nominas[<?=$n;?>][cuota_sindical]" type="checkbox" value="1" <?php echo ($nomina->cuota_sindical)?'checked':''; ?> /></td>
...
<script type="text/javascript">
	$("#btn_nomina").click('input',function() {
        ...
        $('table.nominas tbody tr:last-child select').each(function() {
    		var nameAttr = $(this).attr('name');
            var newIndex = parseInt(nameAttr.replace(/[^\d]/g, ''))+1;
            $(this).attr('name',nameAttr.replace(/\d/,newIndex));
        });
    ...
    function borrar_archivo(control) {
       $.ajax({
       		headers: {'X-CSRF-Token': <?= json_encode($this->request->getParam('_csrfToken')); ?>},
            url: '<?php echo $url?>/empresas/borrar-archivo/',
            cache: false,
            type: 'POST',
            dataType: 'json',
            data: {archivo_id : $(control).data("id") },
            success: function (data) {                    
                if(data.result)
                {                            
                    $(control).closest('tr').remove();
                } else {
                    alert ("error, no pudo borrar archivo");                   
                }
            }
        });  
    };
